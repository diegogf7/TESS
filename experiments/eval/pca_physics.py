...
markers = {"APERIODIC": "o", "CONTACT_ROT": "s", "DSCT_BCEP": "^", "ECLIPSE": "v", "GDOR_SPB": "D", "INSTRUMENT/JUNK": "P", "RRLYR_CEPH": "X", "SOLARLIKE": "*"}
colors = {"APERIODIC": "red", "CONTACT_ROT": "blue", "DSCT_BCEP": "green", "ECLIPSE": "orange", "GDOR_SPB": "black", "INSTRUMENT/JUNK": "grey", "RRLYR_CEPH": "yellow", "SOLARLIKE": "purple"}
...
for c in np.unique(labels):
    m = labels == c
    name = CLASSES[c]
    plt.scatter(embedding[m, 0], embedding[m, 1], s=4, alpha = 0.2, label = name, marker= markers[name], color = colors[name])

plt.legend(markerscale =3, fontsize = 8)
plt.xlabel(f"PC1 ({ev[0]:.1%} var)")
plt.ylabel(f"PC2 ({ev[1]:.1%} var)")
plt.title("Latent PCA (Colored by our Classes)")
plt.savefig(f"{OUT_DIR}/pca_by_class.png", dpi=150, bbox_inches="tight")
plt.close()
...
plt.xlabel(f"PC1 ({ev[0]:.1%} var)")
plt.ylabel(f"PC2 ({ev[1]:.1%} var)")
plt.title("Latent PCA (Colored by our Sectors)")
plt.savefig(f"{OUT_DIR}/pca_by_sector.png", dpi=150, bbox_inches="tight")
plt.close()
...
plt.xlabel(f"PC1 ({ev[0]:.1%} var)")
plt.ylabel(f"PC2 ({ev[1]:.1%} var)")
plt.title("Latent PCA (Colored by light curve length)")
plt.savefig(f"{OUT_DIR}/pca_by_length.png", dpi=150, bbox_inches="tight")
plt.close()

print(f"saved pca_by_class.png, pca_by_sector.png, pca_by_length.png to {OUT_DIR}")
