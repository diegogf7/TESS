...
dataset = DisentanglementDataset(DATA_PATH, grid_length =1024, descriptor_path="/orcd/scratch/orcd/006/diegogon/phyts/TESS/descriptors_train.npy")
dataloader = DataLoader(dataset, batch_size = BATCH_SIZE, shuffle = True, num_workers = 4)

val_dataset = DisentanglementDataset(VAL_PATH, grid_length = 1024, descriptor_path="/orcd/scratch/orcd/006/diegogon/phyts/TESS/descriptors_val.npy")
...
for epoch in range(EPOCHS):
    ...
    for batch_idx, (anchor_flux, anchor_mask, same_star_flux, same_star_mask, same_sector_flux, same_sector_mask) in enumerate(dataloader):
        ...
        prediction, z_target = model(same_sector_flux, same_sector_mask, anchor_flux, anchor_mask)
    ...
    torch.save(model.state_dict(), '/orcd/scratch/orcd/006/diegogon/checkpoints/training_jepa1.pth')