"""
UMAP of the trained LatentJEPA latent (cluster), colored by SECTOR and by CLASS.
Visual companion to eval_jepa.py.

    python -m src.bot_folder.umap_jepa
"""
...
from src.bot_folder.latent_jepa import build_latent_jepa
...
for c in np.unique(labels):
    m = labels == c
    plt.scatter(embedding[m, 0], embedding[m, 1], s = 18, alpha=0.6, marker = MARKERS[c], label=CLASSES[c])
...
print(f"saved jepa_umap_by_sector.png and jepa_umap_by_class.png to {OUT_DIR}")
