...
class Patches(nn.Module):
    def __init__(self, patch_size = 16, dimension_vector = 256): #going to divy up into 64 patches of 16 frames. 
...
class Positions(nn.Module):
    def __init__(self, patches = 64, dimension_vector = 256):
...
class Transformer_Encoder(nn.Module):
    def __init__(self, dimension_vector = 256, number_heads = 4, mlp_dim = 512):
...
class Transformer_Decoder(nn.Module):
    def __init__(self, dimension_vector = 256, patch_size = 16):
        super(Transformer_Decoder, self).__init__()

        self.EncoderLayer = nn.TransformerEncoderLayer(dimension_vector, nhead = 8, dim_feedforward = 256, batch_first = True)
...
class masking_autoencoder(nn.Module):
    def __init__(self, patch_size = 16, dimension_vector = 256, number_patches=64, mask_ratio= 0.75):
        ...
        self.encoder = Transformer_Encoder(dimension_vector, 4, 512)
