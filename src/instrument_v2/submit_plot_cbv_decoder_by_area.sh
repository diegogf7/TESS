...
#SBATCH --mem=64G
#SBATCH -t 01:00:00
#SBATCH -o cbv_by_area_%j.out

# Validation-only diagnostic: does an area's training-star count explain the
# low-Q1 tail of the K=8 CBV-weight decoder? Nothing is trained or modified.
#   sbatch -p ou_mki_gpu --exclude=node2001 src/instrument_v2/submit_plot_cbv_decoder_by_area.sh
set -euo pipefail
...
# matched-experiment config + existing artifacts (all frozen, reused)
export SEED=${SEED:-0}
...
echo "  STAGE_B_CKPT  : $CKPT_DIR/group_cbv_mlp_g${GROUP_SIZE}_r${CBV_RANK}_mv${MIN_VALID_STARS}_s${SEED}_best.pth"
echo "    sha256      : $(sha "$CKPT_DIR/group_cbv_mlp_g${GROUP_SIZE}_r${CBV_RANK}_mv${MIN_VALID_STARS}_s${SEED}_best.pth")"
echo "  DECODER_CKPT  : $DECODER_CKPT"
echo "    sha256      : $(sha "$DECODER_CKPT")"
echo "  CBV_BASES_NPZ : $CBV_BASES_NPZ"
echo "    sha256      : $(sha "$CBV_BASES_NPZ")"
echo "========================================================"
nvidia-smi || true
...
"$PY" -m src.instrument_v2.plot_cbv_decoder_by_area
echo "=== DONE cbv_by_area ==="
